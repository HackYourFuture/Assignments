...
function draw(cell, context) {
  ...
  if (cell.alive) {
    const opacity = Math.min(4, cell.lifeTime) / 4;
...
function main() {
  ...
  const { createGrid, gameLoop } = createGame(context, numRows, numColumns);

  // Create initial grid
  createGrid();

  // Kick-start the first generation
  window.requestAnimationFrame(() => gameLoop());
}
