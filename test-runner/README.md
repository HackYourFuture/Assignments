...
| `<module>/Week𝑛/@assigment` | This folder (notice the leading `@`-sign in the name) is only used during development and maintenance of this repo. Working solutions to exercises can be placed in this folder to test the "happy" path of the unit tests. An `@assigment` folder is used in place of a regular `assignment` folder when a unit test is run with the command: `npm run testalt`.<br><br>Note that `@assignment` folders should not be commited and are therefore included in `.gitignore`. |
