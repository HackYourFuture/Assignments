...
function selectRandomly(arrays) {
  ...
  const selectRandom = arrays[random];
  return selectRandom;
...
function tellFortune(numKids, partnerName, locations, jobTitles) {
  ...
  married to ${parents} with ${kids} kids.`;
...
function main() {
  ...
  const partnerNames = ['rediet', 'nahom', 'yabsira', 'yasmine', 'mickey'];

  const locations = ['deventer', 'amsterdam', 'rotterdam', 'utrecht', 'den bosch'];

  const jobTitles = ['doctor', 'engineer', 'web developer', 'financial manager', 'dentist'];
